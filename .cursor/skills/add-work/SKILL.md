...
2. 打开 http://127.0.0.1:5681#works 或站点导航里的「写内容」，切到「作品」
3. 填标题、英文 slug、年份、摘要、技术栈；封面可上传图片，或从固定配色里选一套并填字母
4. 用工具栏写 Markdown 正文（问题 / 方案 / 要点可当二级标题；可插图）
5. 点「保存到仓库」，生成 `src/content/works/<slug>.md`
6. 预览 http://127.0.0.1:5680/work/<slug>/
7. 提交 git 即发布。不要从工坊里执行 git 推送，除非用户明确要求。

中文标题必须给英文 slug。封面优先用上传的图片；没有图时用固定配色色块。勾选「精选」会成为首页大卡。
