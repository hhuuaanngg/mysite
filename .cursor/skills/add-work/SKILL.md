...
3. 填标题、英文 slug、年份、摘要、问题、方案、要点、技术栈，调封面字母和颜色
4. 点「保存到仓库」，生成 `src/content/works/<slug>.json`
5. 预览 http://127.0.0.1:3100/work/<slug>/
6. 提交 git 即发布。不要从工坊里执行 git 推送，除非用户明确要求。

中文标题必须给英文 slug。封面是色块+字母，不是照片。勾选「精选」会出现在作品列表最前。
