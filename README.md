...
打开 [http://127.0.0.1:3100](http://127.0.0.1:3100)。导航里会出现 **写内容**，进入本机工坊（[http://127.0.0.1:4310](http://127.0.0.1:4310)）。文章和作品都在这里可视化新建、编辑。
...
本机可视化：工坊切到 **作品**（[http://127.0.0.1:4310/#works](http://127.0.0.1:4310/#works)）→ 填标题、年份、问题、方案、要点和技术栈 → 调封面色块 → **保存到仓库**。会生成：

- `src/content/works/<slug>.json`

站点预览：`http://127.0.0.1:3100/work/<slug>/`。确认无误后提交 git，部署即发布。

中文标题必须给英文 slug。封面是色块+字母，不用上传照片。
